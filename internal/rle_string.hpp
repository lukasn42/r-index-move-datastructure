...
#ifndef RLE_STRING_MDS_HPP_
#define RLE_STRING_MDS_HPP_
...
public:
	...
	rle_string(string &input, std::vector<uint32_t> &run_start_positions, ulint B = 2){
		...
		uint j = 1; // current position in run_start_positions

		for(ulint i=1;i<input.size();++i){

			if(i == run_start_positions[j]){
				j++;
		...
		assert(run_heads_s.size()==R);
		assert(R==run_start_positions.size());
